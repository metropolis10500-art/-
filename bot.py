...
def format_profile(user):
    ...
    # HTML экранирование чтобы не падал парсер
    ...
    return f"{gender_emoji} <b>{name}</b>, {user.get('age', '?')} {prem_badge}\n🏙️ {city}\n\n📝 {bio}"
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user and user.get("is_active"):
        await message.answer(f"💘 <b>С возвращением в LoveSpark!</b>\n\nТвоя идеальная пара уже ждёт тебя.", reply_markup=main_menu_kb(get_premium_status(user)), parse_mode=ParseMode.HTML)
        return
        
    await message.answer(f"💘 <b>LoveSpark - Бот знакомств</b> 💘\n\nПривет! Я помогу тебе найти пару по всей России!\n\nДля начала скажи, как тебя зовут?")
    await state.set_state(Registration.name)

@dp.message(Registration.name)
async def process_name(message: Message, state: FSMContext):
    if not (2 <= len(message.text) <= 30): return await message.answer("Имя должно быть от 2 до 30 символов. Попробуй еще раз:")
...
async def process_age(message: Message, state: FSMContext):
    ...
    except ValueError: return await message.answer("Введи корректный возраст цифрами (от 16 до 100):")
...
async def process_gender(message: Message, state: FSMContext):
    ...
    if message.text not in genders: return await message.answer("Пожалуйста, выбери из предложенных кнопок:", reply_markup=gender_kb())
...
async def process_looking_for(message: Message, state: FSMContext):
    ...
    if message.text not in looks: return await message.answer("Пожалуйста, выбери из меню:", reply_markup=looking_for_kb())
...
async def process_bio(message: Message, state: FSMContext):
    if len(message.text) < 5: return await message.answer("Описание слишком короткое. Напиши еще пару слов:")
    await state.update_data(bio=message.text[:500])
    data = await state.get_data()
    
    await message.answer_photo(photo=data['photo'], caption=f"📋 <b>Предпросмотр анкеты:</b>\n\n{format_profile(data)}\n\nВсё верно?", parse_mode=ParseMode.HTML)
...
async def confirm_reg(message: Message, state: FSMContext):
    ...
    await message.answer("🎉 <b>Анкета создана! Добро пожаловать в LoveSpark!</b>", reply_markup=main_menu_kb(False), parse_mode=ParseMode.HTML)
    await state.clear()
...
async def find_pair(message: Message):
    ...
    if not get_premium_status(user) and user.get("likes_today", 0) >= FREE_LIKES_PER_DAY:
        return await message.answer("😔 Лимит лайков на сегодня исчерпан!\n\n💎 Купи Премиум для безлимита!", reply_markup=main_menu_kb(False))

    profile = await get_random_profile(user['telegram_id'], user['looking_for'], user['city'])
    if not profile: return await message.answer("Пока нет подходящих анкет. Загляни попозже!", reply_markup=main_menu_kb(get_premium_status(user)))
...
async def my_matches_cmd(message: Message):
    ...
    if not matches: return await message.answer("💔 У тебя пока нет мэтчей.\nСтавь лайки понравившимся людям!")
    ...
    for m in matches[:10]:
        ...
        await message.answer_photo(photo=m['photo'], caption=f"💕 <b>{name_safe}</b>, {m['age']}\n🏙️ {city_safe}", reply_markup=match_actions_kb(m['id'], p_id), parse_mode=ParseMode.HTML)
...
async def show_stats_cmd(message: Message):
    ...
    user = await get_user(message.from_user.id)
    await message.answer(text, parse_mode=ParseMode.HTML, reply_markup=main_menu_kb(get_premium_status(user)))
...
async def my_premium_cmd(message: Message):
    user = await get_user(message.from_user.id)
    if not get_premium_status(user): return await message.answer("У тебя нет Премиум подписки.", reply_markup=main_menu_kb(False))
    until = datetime.datetime.fromisoformat(user["premium_until"])
    await message.answer(f"👑 <b>Твой Премиум</b>\n\n📅 Действует до: {until.strftime('%d.%m.%Y')}\n⏳ Осталось: {(until - datetime.datetime.now()).days} дней", parse_mode=ParseMode.HTML)

@dp.message(F.text == "❓ Помощь")
async def help_cmd(message: Message):
    text = f"❓ <b>Помощь</b>\n\n1. Нажимай «❤️ Найти пару»\n2. Ставь лайки\n3. При взаимном лайке вы сможете общаться\n\n<b>Команды:</b>\n/start - Перезапуск\n/delete - Удалить профиль\n\nЛимиты бесплатно: {FREE_LIKES_PER_DAY} лайков, {FREE_MESSAGES_PER_DAY} сообщений."
...
async def process_like(callback: CallbackQuery):
    ...
    if is_mutual:
        ...
        await callback.message.answer(f"💕 <b>Взаимный мэтч!</b>\nВы и {partner_name} понравились друг другу!", reply_markup=match_actions_kb(0, target_id), parse_mode=ParseMode.HTML)
        try: await bot.send_message(target_id, f"💕 <b>Новый мэтч!</b>\n{my_name} лайкнул(а) тебя взаимно!", reply_markup=match_actions_kb(0, user['telegram_id']), parse_mode=ParseMode.HTML)
...
async def process_superlike(callback: CallbackQuery):
    target_id = int(callback.data.split("_")[1])
    user = await get_user(callback.from_user.id)
    if not get_premium_status(user): return await callback.answer("Супер-лайки только для Премиум!", show_alert=True)
    
    is_mutual = await add_like(user['telegram_id'], target_id)
    if is_mutual:
        partner = await get_user(target_id)
        await callback.message.answer(f"⭐ <b>Супер-мэтч!</b>\nВы и {html.escape(partner['name'])} - мэтч!", reply_markup=match_actions_kb(0, target_id), parse_mode=ParseMode.HTML)
    else:
...
async def start_chat_cb(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(match_id=match_id, partner_id=int(p_id))
    await callback.message.answer(f"💬 <b>Чат с {html.escape(partner['name'])}</b>\nОтправляй текст, фото, видео или стикеры.\nВыход: /exit", reply_markup=chat_actions_kb(match_id), parse_mode=ParseMode.HTML)
...
async def write_message_from_profile(callback: CallbackQuery, state: FSMContext):
    ...
    callback.data = f"chat_{match['id']}_{target_id}"
...
async def view_profile_cb(callback: CallbackQuery):
    ...
    if p: await callback.message.answer_photo(photo=p['photo'], caption=format_profile(p), parse_mode=ParseMode.HTML)
...
async def process_chat_message(message: Message, state: FSMContext):
    ...
    elif message.video: content_type, file_id = "video", message.video.file_id
    elif message.video_note: content_type, file_id = "video_note", message.video_note.file_id
    ...
    try:
        if content_type == "text": await bot.send_message(partner_id, f"💬 <b>{name_safe}:</b>\n{content_safe}", parse_mode=ParseMode.HTML)
        elif content_type == "photo": await bot.send_photo(partner_id, file_id, caption=f"📸 <b>{name_safe}</b>", parse_mode=ParseMode.HTML)
        elif content_type == "voice": await bot.send_voice(partner_id, file_id, caption=f"🎙️ <b>{name_safe}</b>", parse_mode=ParseMode.HTML)
        elif content_type == "sticker": await bot.send_sticker(partner_id, file_id)
        elif content_type == "video": await bot.send_video(partner_id, file_id, caption=f"🎬 <b>{name_safe}</b>", parse_mode=ParseMode.HTML)
        elif content_type == "video_note": await bot.send_video_note(partner_id, file_id)
    except Exception:
...
async def exit_chat(message: Message, state: FSMContext):
    await state.clear()
    user = await get_user(message.from_user.id)
    await message.answer("🔙 Вы вышли из чата.", reply_markup=main_menu_kb(get_premium_status(user)))
...
async def select_premium(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"💎 <b>{tariff['name']}</b>\n💰 К оплате: {tariff['price']}₽\n📝 {tariff['description']}", reply_markup=payment_kb(url, label), parse_mode=ParseMode.HTML)
...
async def edit_process(callback: CallbackQuery, state: FSMContext):
    ...
    prompts = {"photo": "📸 Отправь новое фото:", "name": "📝 Введи новое имя:", "age": "🔢 Введи возраст:", "city": "🏙️ Город:", "bio": "📝 О себе:", "looking": "👀 Кого ищешь?"}
    await state.update_data(edit_field=field)
    await state.set_state(EditProfile.new_value)
    
    if field == "city": await callback.message.answer(prompts[field], reply_markup=city_kb())
    elif field == "looking": await callback.message.answer(prompts[field], reply_markup=looking_for_kb())
...
async def save_edit(message: Message, state: FSMContext):
    ...
    if field == "photo":
        ...
        await update_user(user['telegram_id'], name=message.text[:30])
    elif field == "age":
        if not message.text.isdigit(): return await message.answer("Нужно число:")
        await update_user(user['telegram_id'], age=int(message.text))
    elif field == "city":
        await update_user(user['telegram_id'], city=message.text[:50])
    elif field == "bio":
        await update_user(user['telegram_id'], bio=message.text[:500])
    elif field == "looking":
        looks = {"👨 Мужчин": "male", "👩 Женщин": "female", "👫 Всех": "both"}
        if message.text not in looks: return await message.answer("Используй кнопки:", reply_markup=looking_for_kb())
        await update_user(user['telegram_id'], looking_for=looks[message.text])

    await message.answer("✅ Сохранено!", reply_markup=main_menu_kb(get_premium_status(user)))
    await state.clear()
...
async def back_menu_cb(callback: CallbackQuery):
    user = await get_user(callback.from_user.id)
    await callback.message.delete()
    await callback.message.answer("Главное меню", reply_markup=main_menu_kb(get_premium_status(user)))
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Успешно доставлено: {success}")
    await state.clear()

@dp.message(Command("delete"))
async def delete_cmd(message: Message):
    await message.answer("⚠️ Точно удалить анкету?", reply_markup=confirm_delete_kb())

@dp.callback_query(F.data == "confirm_delete")
async def confirm_del_cb(callback: CallbackQuery):
    await update_user(callback.from_user.id, is_active=0)
    await callback.message.answer("😢 Анкета удалена. Возвращайся!", reply_markup=ReplyKeyboardRemove())

@dp.callback_query(F.data == "cancel_delete")
async def cancel_del_cb(callback: CallbackQuery):
    user = await get_user(callback.from_user.id)
    await callback.message.answer("Отлично!", reply_markup=main_menu_kb(get_premium_status(user)))
