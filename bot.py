...
PREMIUM_PRICE = int(os.getenv("PREMIUM_PRICE", 299))
BOOST_PRICE = int(os.getenv("BOOST_PRICE", 99))
SUPERLIKE_PACK_PRICE = int(os.getenv("SUPERLIKE_PACK_PRICE", 49))
...
def get_db():
    return sqlite3.connect('lovebot.db')
...
def chat_menu(match_id: int, partner_id: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="◀️ Закрыть чат", callback_data="my_chats")]
    ])
...
async def cmd_start(message: types.Message):
    ...
    if not user[3]:  # name is null
        await message.answer(
            "💕 <b>Добро пожаловать в LoveBot Pro!</b>\n\n"
            "Самый функциональный бот знакомств в Telegram.\n\n"
    ...
    await message.answer(
        f"💕 <b>LoveBot Pro</b>\n\n"
        f"🔍 Найдите свою половинку\n"
        f"💕 Ставьте лайки и получайте матчи\n"
        f"💌 Общайтесь в чатах\n"
        f"🎁 Отправляйте подарки\n\n"
        f"{'✅ Верификация пройдена' if user[9] else '⚠️ Пройдите верификацию для доверия'}\n"
...
async def set_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "🔍 Смотреть анкеты\n"
        "💕 Ставить лайки\n"
        "💌 Общаться при взаимности\n\n"
        "💡 Совет: Пройдите верификацию, чтобы получать больше лайков!",
...
async def browse_profiles(callback: types.CallbackQuery):
    ...
    # Бустированные профили показываем первыми
    query += " ORDER BY CASE WHEN profile_boosted_until > datetime('now') THEN 0 ELSE 1 END, RANDOM()"
    query += " LIMIT 1"
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="❌", callback_data=f"skip_{target_id}"),
            InlineKeyboardButton(text="💕", callback_data=f"like_{target_id}"),
            InlineKeyboardButton(text="⭐", callback_data=f"superlike_{target_id}")
        ],
        [
    ...
    await callback.message.answer_photo(photo_id, caption=caption, reply_markup=kb, parse_mode="HTML")
    await callback.answer()
    ...
    if mutual:
        ...
        try:
            await bot.send_message(
                ...
                f"Вы понравились друг другу! Начните общение 💌",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await callback.answer("💕 Матч! Открывайте чат!", show_alert=True)
    else:
        ...
        if check_premium(target_id):
            ...
            try:
                await bot.send_message(
                    target_id,
                    f"💕 <b>Новый лайк!</b>\n\n"
                    f"Пользователь заинтересовался вами!\n"
...
async def my_chats(callback: types.CallbackQuery):
    ...
    if not matches:
        await callback.message.edit_text(
            "💌 Пока нет матчей.\n\n"
            "Ставьте лайки, чтобы найти пару!",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔍 Смотреть анкеты", callback_data="browse")],
                [InlineKeyboardButton(text="◀️ Назад", callback_data="main_menu")]
            ])
...
async def open_chat(callback: types.CallbackQuery):
    ...
    if not messages:
        text += "<i>Начните общение! Напишите сообщение ниже 👇</i>"
    
    await callback.message.delete()
    await callback.message.answer_photo(
        photo_id,
        caption=text,
        reply_markup=chat_menu(match_id, partner_id),
        parse_mode="HTML"
    )
    await callback.answer()
    
    # Сохраняем состояние для приёма сообщений
    # Используем FSM для отслеживания активного чата
    # (в реальном боте нужен более сложный механизм)
...
async def handle_chat_message(message: types.Message):
    ...
    # Проверяем, есть ли активный чат (упрощённо — через контекст)
    # В реальном боте нужна система активных чатов
    
    # Здесь заглушка — в полной версии нужен менеджер сессий
    await message.answer(
        "💬 Чтобы написать сообщение, откройте чат через кнопку ниже.",
        reply_markup=main_menu(user_id)
    )
...
async def send_gift(callback: types.CallbackQuery):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT name, emoji, price FROM gift_types WHERE id = ?", (gift_id,))
    gift = c.fetchone()
    c.execute("SELECT name FROM users WHERE user_id = ?", (target_id,))
    target_name = c.fetchone()[0]
    conn.close()
    ...
    await callback.message.edit_text(
        ...
        f"Подарок: {emoji} {name}\n"
        f"Получатель: {target_name}\n"
        f"Цена: {price} ₽\n\n"
        f"Для покупки переведите {price} ₽ на карту:\n"
        f"<code>2200 1234 5678 9012</code>\n"
        f"И отправьте скриншот администратору.\n\n"
        f"После оплаты подарок будет доставлен мгновенно!",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def premium_info(callback: types.CallbackQuery):
    ...
    text = (
        ...
        "✅ <b>Видеть всех, кто вас лайкнул</b>\n"
        ...
        f"💰 <b>{PREMIUM_PRICE} ₽ / 30 дней</b>\n\n"
        f"Для покупки переведите {PREMIUM_PRICE} ₽ на карту:\n"
        f"<code>2200 1234 5678 9012</code>\n"
        f"И отправьте скриншот администратору."
    )
...
async def buy_boost(callback: types.CallbackQuery):
    text = (
        ...
        f"💰 <b>{BOOST_PRICE} ₽ / 24 часа</b>\n\n"
        f"Для покупки переведите {BOOST_PRICE} ₽ на карту:\n"
        f"<code>2200 1234 5678 9012</code>"
    )
...
async def my_likes(callback: types.CallbackQuery):
    ...
    if not is_premium:
        count = len(likes)
        await callback.message.edit_text(
            f"💕 <b>Вас лайкнули {count} человек(а)</b>\n\n"
...
async def my_profile(callback: types.CallbackQuery):
    ...
    await callback.message.answer_photo(photo_id, caption=caption, reply_markup=profile_menu(), parse_mode="HTML")
    await callback.answer()
...
async def settings(callback: types.CallbackQuery):
    ...
    kb.append([InlineKeyboardButton(text="🗑 Удалить профиль", callback_data="delete_profile")])
    kb.append([InlineKeyboardButton(text="◀️ Назад", callback_data="main_menu")])
...
async def admin_panel(callback: types.CallbackQuery):
    ...
    text = (
        f"🔑 <b>Админ-панель LoveBot Pro</b>\n\n"
...
async def process_premium_activation(message: types.Message, state: FSMContext):
    ...
    c.execute("INSERT INTO purchases (user_id, product_type, product_name, price, status, activated_at) VALUES (?, ?, ?, ?, 'completed', ?)",
              (user_id, 'premium', f'Premium {days} days', PREMIUM_PRICE, datetime.now()))
    conn.commit()
    conn.close()
